...
disp(' ')
disp('Zero-crossing of the damping')
...
for s=1:n_moden
    if ~isnan(zeroCrossings.forward(s)) %remove NaN values
        ...
        disp([num2str(ZeroCrossingForward.modeNumber(i)),'. Mode (forward): at ', num2str(ZeroCrossingForward.rpm(i)), ' 1/min'])
        i=i+1;
    end
end
% disp(' ')
...
for s=1:n_moden
    if ~isnan(zeroCrossings.backward(s))
        ...
        disp([num2str(ZeroCrossingBackward.modeNumber(i)),'. Mode (backward): at ', num2str(ZeroCrossingBackward.rpm(i)), ' 1/min'])
        i=i+1;
    end
end

if all(isnan(zeroCrossings.forward)) && all(isnan(zeroCrossings.backward))
    disp('No zero-crossing of the damping in the considered range.')
end
