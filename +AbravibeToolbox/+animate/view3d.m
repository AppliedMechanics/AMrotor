...
if ~ismember(get(gca,'Tag'),{'Axes:Single','Axes:Ortho11'})
    return
end
view3dObj  = findobj(allchild(gcf),'Tag','view3dObj');
vdata = get(view3dObj,'UserData');
...
vdata.old_pt = new_pt;
set(view3dObj,'UserData',vdata)
changeQuad('rotXY',dx,dy,coordsys,direction);


% ---------------------------------------------- view3dzFcn ----------
function view3dzFcn
if ~ismember(get(gca,'Tag'),{'Axes:Single','Axes:Ortho11'})
%if ~strcmp(get(gca,'Tag'),'Axes:Single')
    return
end
view3dObj  = findobj(allchild(gcf),'Tag','view3dObj');
vdata = get(view3dObj,'UserData');
...
function changeQuad(action,varargin)
axs     = setdiff(findobj(gcf,'Type','Axes'),gca);

switch action
    case 'zoom'
        for ii=1:length(axs)
            camzoom(axs(ii),1-varargin{1})
        end
    case 'pan'
        for ii=1:length(axs)
            campan(axs(ii),-varargin{1},-varargin{2},'camera')
        end
    case 'rotXY'
        if length(axs)>2
            return;
        ...
        for ii=1:length(axs)
            pos  = get(axs(ii),'cameraposition' );
            targ = get(axs(ii),'cameratarget'   );
            dar  = get(axs(ii),'dataaspectratio');
            up   = get(axs(ii),'cameraupvector' );
            [newPos newUp] = camrotate(pos,targ,dar,up,-varargin{1},-varargin{2},varargin{3},varargin{4});
            set(axs(ii),'cameraposition', newPos, 'cameraupvector', newUp);
        end
    case 'rotZ'
        if length(axs)>2
            return;
