%% ======================================================================
% Rotor

cnfg.cnfg_rotor.name = 'Akademisches Beispiel';
cnfg.cnfg_rotor.shear_def = 1;     % 1 = Timoshenko or 0 = Bernoulli beam theoy

%rotor geometry             [z1,da,di;   z2,da,di]  
cnfg.cnfg_rotor.rotor_dimensions = [0,8,0;385,8,0;415,150,0;800,8,0]*1e-3; %[m] %Angefangen bei 0, die Durchmessersprünge am ende des Abschnites eingeben
cnfg.cnfg_rotor.shear_factor   = 0.9;  %!!!!!!!!!wird Automatisch korregiert wenn Kreisring.
                                %!!!!!!!!!Schubkorrekrutfaktor für Kreisquerschnitt 0.9[-], Kreisring 0.5[-]

cnfg.cnfg_rotor.E_module = 211e9;  %[N/m^2]
cnfg.cnfg_rotor.density  = 7446;   %[kg/m^3]
cnfg.cnfg_rotor.poisson  = 0.3;    %steel 0.27...0.3 [-]

cnfg.cnfg_rotor.dz=0.01;           %set elemet length for meshing

    
%% ========================================================================
% Massescheiben
cnfg.cnfg_disc=[];

%% ========================================================================
% Sensors
cnfg.cnfg_sensor=[];

  cnfg.cnfg_sensor(1).name = 'Positionssensor Scheibe';
  cnfg.cnfg_sensor(1).position=0.4;
  cnfg.cnfg_sensor(1).type=1;

  cnfg.cnfg_sensor(2).name='Kraftsensor links';
  cnfg.cnfg_sensor(2).position=0.1;
  cnfg.cnfg_sensor(2).type=2;
  %
%   cnfg.cnfg_sensor(3).name='Kraftsensor rechts';
%   cnfg.cnfg_sensor(3).position=0.7;
%   cnfg.cnfg_sensor(3).type=2;
  
  cnfg.cnfg_sensor(3).name = 'Positionssensor links';
  cnfg.cnfg_sensor(3).position=0.1;
  cnfg.cnfg_sensor(3).type=1;
  
  cnfg.cnfg_sensor(4).name = 'Positionssensor rechts';
  cnfg.cnfg_sensor(4).position=0.7;
  cnfg.cnfg_sensor(4).type=1;
  
  
  

%% ========================================================================
% Lager
cnfg.cnfg_lager=[];

cnfg.cnfg_lager(1).name = 'Linkes Simple Lager';
cnfg.cnfg_lager(1).position=0.100;                        %[m]
cnfg.cnfg_lager(1).type=1;
cnfg.cnfg_lager(1).stiffness=5e7;                     %[N/m]


cnfg.cnfg_lager(2).name = 'Rechtes Simple Lager';
cnfg.cnfg_lager(2).position=0.700;                        %[m]
cnfg.cnfg_lager(2).type=1;
cnfg.cnfg_lager(2).stiffness=5e7;%7.0e5;              %[N/m]



%% ========================================================================
% Kraft in feste Richtung
cnfg.cnfg_force_const_fix=[];

% Unwuchten
cnfg.cnfg_unbalance=[];


%% Infos
% %KoSy:
%        Z-Längsachse des Rotors
%        X-Z-Ebene
%        verschiebung in X1       X1
%        drehung um Y1            Beta_1
%        verschiebung in X2       X2
%        drehung um Y2            Beta_2
%                                  :
%                                 Xn
%        
%        Y-Z-Ebene
%        verschiebung in Y1       Y1
%        drehung um Z1            Alpha_1
%        verschiebung in Y2       Y3
%        drehung um Z2            Alpha_2
%                                   :
%                                 Yn
%                                 Alpha_n

