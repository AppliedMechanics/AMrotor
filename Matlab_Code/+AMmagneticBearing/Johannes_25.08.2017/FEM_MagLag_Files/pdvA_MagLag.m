function [ Kraefte, Energien, GitterZellenArray ] = pdvA_MagLag( Position, SpulenStrom, varMu)
%   Name: PDVA_MagLag.m
...
global debugMode plotGeometry plotMesh plotVektorPot  % Globale Variablen dienen als Schalter
...
for i=1:3           % drei Simulationen fuer Prinzip der virtuellen Verschiebung
    ...
    if varMu
        ...
        [WNonLin, A,model,GitterZellen]=FEM_MagLag(Position,VerschiebewegMat(i,:),SpulenStrom,1);  % Zusaetzlich FEM-Simulation mit nichtlinearer Permeabilitaet      
        WNonLinMat(i)=WNonLin; 
         if (plotGeometry==1) || (plotMesh==1) || (plotVektorPot==1)
             Plots_MagLag
         end
    else
        [W,A,model,GitterZellen]=FEM_MagLag(Position, VerschiebewegMat(i,:),SpulenStrom,0);
        WMat(i) = W;
    end    
end    
