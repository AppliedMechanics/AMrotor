...
if nonlinMu  
    cCoef = @(region,state) (mu_0*(mu_Eisen./(1+0.05*(state.ux.^2 +state.uy.^2))+200));    % nicht konstanter Koeffizient C (Permeabilitaet)
...
% 
% generateMesh(model);
% generateMesh(model,'GeometricOrder','quadratic' ,'MesherVersion','R2013a','Hgrad',1.15);
% generateMesh(model,'GeometricOrder','quadratic','Hgrad',1.15);
% generateMesh(model,'Hgrad',1.2);
% generateMesh(model,'MesherVersion','R2013a','Hgrad',1.25);
generateMesh(model,'MesherVersion','R2013a');
