%% ======================================================================
% Abgestimmt mit Daten aus dem CAD, zur Abstimmmung der Geometrie
% nur Magnetlagerzapfen ohne Spannhuelsen
...
% Rotor Config
% Geometrie nur Magnetlagerzapfen ohne Spannhuelsen
% Abschnitte A,B,C,D,E,F als 1,2,3,4,5,6
A = [4.70228E-05, 3.392837e-4, 2.194411e-3, 3.655756e-4, 2.733201e-3, 0.014902];
I = [1.76117E-10, 1.227254e-7, 4.698135e-7, 6.90575e-8, 3.263916e-6, 1.840829e-5];
[ra,ri] = calculate_radii_from_IA(A,I);
z = [0, 64-1, 94-1, 132+1, 162+1, 335, 350, 360, 366, 376, 391, 574, 604, 642, 672, 702]*1e-3;%Geometrieknoten z-Positionen

cnfg.cnfg_rotor.geo_nodes = { ...
    [z(2)-z(2) ra(2) ri(2)], [z(3)-z(2) ra(2) ri(2)], ... % Abschnitt B
    [z(3)-z(2) ra(3) ri(3)], [z(4)-z(2) ra(3) ri(3)], ... % Abschnitt C
    [z(4)-z(2) ra(2) ri(2)], [z(5)-z(2) ra(2) ri(2)], ... % Abschnitt B
    }; % Endknoten
clear A I ra ri z
