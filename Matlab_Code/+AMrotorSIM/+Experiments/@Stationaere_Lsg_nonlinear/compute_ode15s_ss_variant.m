     function compute_ode15s_ss_variant(obj)
       ...
       for rpm = obj.drehzahlen 
        ...
        %%init Vector
        ndof = length(obj.rotorsystem.rotor.matrices.M);
...
%         options = odeset('AbsTol', 1e-5, 'RelTol', 1e-5); % ohne live-plot
        ...
        sol = ode15s(@integrate_function_variant,obj.time,Z0, options, Omega, obj);
        ...
        if any(strcmp({obj.rotorsystem.sensors.type},'ForceLoadPostSensor'))
            if ~isempty(obj.rotorsystem.loads)
                for k = 1:length(obj.time)
                    h_ges(:,k) = h_ges(:,k) + obj.rotorsystem.compute_system_load_variant(obj.time(k), [res.X(:,k); res.X_d(:,k)]);
                end
            end
        end
        
        % fuer SealsNonLinear
        if any(strcmp({obj.rotorsystem.sensors.type},'ForceSealsNonLinearPostSensor'))
            if ~isempty(obj.rotorsystem.sealsNonLinear)
                for sealNonLinear=obj.rotorsystem.sealsNonLinear
                    for k = 1:length(obj.time)
                        [F] = sealNonLinear.get_sealsNonLinear_force(Z(:,k),Zp(:,k),obj.rotorsystem);
                        h_ges(end/2+1:end,k) = h_ges(end/2+1:end,k) + F;
                    end
                end
            end
        end
