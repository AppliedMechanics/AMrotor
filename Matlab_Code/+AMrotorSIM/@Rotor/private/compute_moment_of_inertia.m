...
geometry_par = zeros(Dim(1)-1, 5);
volume_rotor = 0;
JzzNorm = 0; % int r^2 dV, entspricht J auf Einheitsdichte normiert 

for n = 1:Dim(1)-1
%for n = 2:Dim(1)
    geometry_par(n,1) = pi*(0.25*rotorpar.rotor_dimensions(n+1,2)^2-0.25*rotorpar.rotor_dimensions(n+1,3)^2); % Flaeche
    geometry_par(n,2) = pi*((0.5*rotorpar.rotor_dimensions(n+1,2))^4/4-(0.5*rotorpar.rotor_dimensions(n+1,3))^4/4); % I_xi
    ...
    k = rotorpar.rotor_dimensions(n+1,3);
